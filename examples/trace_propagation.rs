...
use otel_tracing::{
    counter, debug_log, error_log, facade::MetricUnit, gauge, histogram, info_log, span, telemetry::{init_datadog, shutdown}, warn_log, with_async_span, with_span
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_histogram = Arc::new(histogram!("rust_test_service.response_time", 
        "Test response time histogram", 
        MetricUnit::Time(otel_tracing::facade::TimeUnit::Millisecond)
    ));
    ...
    let result = with_async_span!("main_process", async {
        ...
        let data = with_async_span!("prepare_data", async {
            ...
            duration_histogram.record(duration_ms, vec![("step".to_string(), "prepare_data".into())]);
        ...
        for i in 0..3 {
            ...
            let handle = spawn_with_context(async move {
                ...
                with_async_span!("background_task", 
                    ...
                    async {
                        ...
                        let result = with_async_span!("process_data", async {
                            ...
                            duration_histogram.record(duration_ms, vec![
                        ...
                        duration_histogram.record(task_duration_ms, vec![
        ...
        let final_result = with_span!("aggregate_results", {
            ...
            duration_histogram.record(duration_ms, vec![("step".to_string(), "aggregate_results".into())]);
        ...
        duration_histogram.record(total_duration_ms, vec![("step".to_string(), "total".into())]);
